...
(function($) {
  ...
  $.fn.firstVisit = function(options) {
    if (!window.localStorage) {
      console.warn(
        "localStorage is not defined, add Modernizr referece: https://github.com/Modernizr/Modernizr"
      );
      return;
    }
    ...
    if (settings.currentPathOnly && settings.currentPathStartsWith) {
      if (
        window.location.pathname.indexOf(settings.currentPathStartsWith) != 0
      ) {
    ...
    var getVisitKeyName = function() {
      return settings.currentPathOnly
        ? VISIT_KEY_NAME +
            "_" +
            (settings.currentPathStartsWith
              ? settings.currentPathStartsWith
    ...
    if (!visitKey) {
      ...
      // add class if any
      if (settings.classToAdd) {
        $elements.addClass(settings.classToAdd);
      }
    }
  };
})(jQuery);
