...
if(!isset($_SESSION['objLogin'])){
	...
	$form_url = WEB_URL . "setting/users.php";
	$hval = 0;
	
	if(isset($_POST['txtAdminName'])){
		if($_POST['hdnSpid'] == '0'){		
			$sql="INSERT INTO `tbl_add_uadmin`(`name`,`email`,`password`,`branch_id`) VALUES ('$_POST[txtAdminName]','$_POST[txtAdminEmail]','$_POST[txtAdminPassword]','$_POST[ddlBranch]')";	
			mysql_query($sql, $link);
			mysql_close($link);
		    $url = WEB_URL . 'setting/users.php?m=add';
		    header("Location: $url");
		}
		else{		
			$sql_update="UPDATE `tbl_add_uadmin` set name = '".$_POST['txtAdminName']."',email = '".$_POST['txtAdminEmail']."',password = '".$_POST['txtAdminPassword']."',branch_id = '".$_POST['ddlBranch']."' where aid= '" . (int)$_POST['hdnSpid'] . "'";	
			mysql_query($sql_update, $link);
			mysql_close($link);
		    $url = WEB_URL . 'setting/users.php?m=up';
		    header("Location: $url");
	...
	if(isset($_GET['spid']) && $_GET['spid'] != ''){
		$result = mysql_query("SELECT *,a.added_date as p_added_date,b.branch_name from tbl_add_uadmin a inner join tblbranch b on b.branch_id = a.branch_id where a.aid= '" . (int)$_GET['spid'] . "'",$link);
		...
		if($row = mysql_fetch_array($result)){
			...
			$form_url = WEB_URL . "setting/users.php?id=".$_GET['spid'];
			$hval = $row['aid'];
		}
...
<section class="content-header">
  ...
  <ol class="breadcrumb">
    ...
    <li class="active">Tax Station Admin Setup</li>
  </ol>
</section>
<!-- Main content -->
  ...
  <div class="col-md-12">
    ...
    <div class="box box-info">
      <div class="box-header">
        <h3 class="box-title">Tax Station Admin Setup Form</h3>
      </div>
      <form onSubmit="return validateMe();" action="<?php echo $form_url; ?>" method="post" enctype="multipart/form-data">
        <div class="box-body">
          ...
          <div class="form-group pull-right">
            ...
            &nbsp;
            <input type="reset" onClick="javascript:window.location.href='<?php echo WEB_URL; ?>setting/users.php';" name="btnReset" id="btnReset" value="Reset" class="btn btn-primary"/>
          </div>
        </div>
...
$msg = "";
 if(isset($_GET['delid']) && $_GET['delid'] != '' && $_GET['delid'] > 0){
		$sqlx= "DELETE FROM `tbl_add_uadmin` WHERE aid = ".$_GET['delid'];
		mysql_query($sqlx,$link); 
		$delinfo = 'block';
	}
if(isset($_GET['m']) && $_GET['m'] == 'add'){
	$addinfo = 'block';
	$msg = "Added Tax Station Admin Information Successfully";
}
if(isset($_GET['m']) && $_GET['m'] == 'up'){
	$addinfo = 'block';
	$msg = "Updated Tax Station Admin Information Successfully";
}
?>
      <!-- Main content -->
      <section class="content">
          ...
          <div class="box box-info">
            ...
            <div class="box-body">
              <table class="table sakotable table-bordered table-striped dt-responsive">
                ...
                <tbody>
                  <?php
				$result = mysql_query("SELECT *,a.added_date as p_addted_date,b.branch_name from tbl_add_uadmin a inner join tblbranch b on b.branch_id = a.branch_id order by a.aid DESC",$link);
                      ...
                      <div class="modal-dialog">
                        <div class="modal-content">
                          <div class="modal-header orange_header">
                            ...
                            <h3 class="modal-title">Station Admin Details</h3>
                          </div>
...
<script type="text/javascript">
  function deleteAdmin(Id){
	...
	if(iAnswer){
		window.location = '<?php echo WEB_URL;?>setting/users.php?delid=' + Id;
	}
  }
  </script>
